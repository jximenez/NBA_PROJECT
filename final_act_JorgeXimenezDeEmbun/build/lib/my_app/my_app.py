...
from my_app.my_app_entities.get_players_stats import GetPlayersStats
...
class MyApp:
    ...
    def __init__(self):
        ...
        self.__player_name = 'marc gasol'
        self.__player_season = '2010'
        self.__game_id = '26427'
    ...
    def getting_players_stats(self):
        ...
        if does_exist:
            with open(f'my_app_saved_files/players_requested/{self.__player_name}.json', 'w') as json_player_file:
                json.dump(player_info, json_player_file)

            with open(f'my_app_saved_files/players_requested/{self.__player_name}.json', 'r') as player_file:
                player_info = json.load(player_file)
        
            player_id = player_info['id']
            get_players_stats = GetPlayersStats(self.__base_url, player_id, self.__player_season)
            player_season_stats, season_played = get_players_stats.get_players_stats()
            if season_played:
                with open(f'my_app_saved_files/stats_players_requested/{self.__player_name}_{self.__player_season}.json', 'w') as json_stats_file:
    ...
    def getting_game_info(self):
        ...
        with open(f'my_app_saved_files/stats_players_requested/{self.__player_name}_{self.__player_season}.json', 'r') as season_stats_file:
        ...
        if self.__game_id in season_stats:
            ...
            if exist:
                with open(f'my_app_saved_files/games_info/{self.__player_name}_{self.__player_season}_{self.__game_id}.json', 'w') as games_info_file:
    ...
    def getting_average_season_player_stats(self):
        ...
        if exist:
            with open(f'my_app_saved_files/average_season_stats/{self.__player_name}_{self.__player_season}_AverageStats.json', 'w') as average_stats_file:
